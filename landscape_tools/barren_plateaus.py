...
PaddingType = Literal["identity", "I", "linear_half", "log"]
...
class ExperimentConfig:
    analysis_type: str
    """Type of scaling analysis to run. This selects how qubit 
    counts, layer counts, observables, and padding strategies are combined."""
    ...
    """Initial Pauli string used when observables are 
    generated by padding."""
    padding_types: Sequence[PaddingType] | None = None
    """Padding strategies used to grow the initial Pauli string 
...
def pad_pauli_strings_growth(
    pauli: Pauli | str,
    target_n: int,
    growth: PaddingType = "linear_half",
    log_base: float = 2,
) -> Pauli:
    ...
    def round_value(x: float):
        return int(np.ceil(x))

    def target_number_of_active_paulis(n: int):
        if growth == "linear_half":
            return round_value(n / 2)
        elif growth == "log":
            return 1 if n <= 1 else round_value(np.log(n) / np.log(log_base))
        else:
            raise ValueError("growth must be 'identity', 'I', 'linear_half', or 'log'")

    label = pauli.to_label() if isinstance(pauli, Pauli) else pauli

    n_current = len(label)
    if n_current > target_n:
        raise ValueError(f"Pauli {label} has length {n_current} > target_n={target_n}")
    ...
    n_active_to_add = max(0, target_count - current_count)
    n_active_to_add = min(n_active_to_add, n_padding)
...
def bootstrap_var_diagnostic_1d(
    ...
    Args:
        ...
        L_samples: Array of loss values evaluated for different parameter
            samples. Can have shape ``(N_samples,)`` or ``(N_samples, 1)``.
...
def adaptive_sampling_var(
    ...
    r"""Perform adaptive sampling to estimate the loss variance with
    bootstrap-based error control.
    ...
    Returns:
        ...
        samples; the variance computed over all samples; and a dictionary
        containing bootstrap diagnostic information for the selected observable.
    """
...
def plot_layerwise_qubits(
    ...
    Args:
        results: Mapping from circuit depth to computed statistics.
...
def plot_layerwise_qubits_padding(
    results: dict[tuple[int, Optional[PaddingType]], list[Result]],
    N_layers: Sequence[int],
    ...
    Args:
        results: Mapping indexed by ``(depth, padding_type)`` tuples.
            Each entry must contain dictionaries with at least:
            - "nq": number of qubits
            - "var": variance value
            - "obs": observable identifier
    ...
    padding_to_marker = {
        pad: markers[i % len(markers)] for i, pad in enumerate(padding_types or [None])
    }
    ...
    for lay in N_layers:
        ...
        for pad in padding_types or [None]:
            ...
            pts = sorted(results[key], key=lambda d: d["nq"])
    ...
    handles_L = [
        Line2D([0], [0], color=layer_to_color[lay], lw=2, label=rf"$L={lay}$")
        for lay in N_layers
    ]
...
def plot_joint_scaling_padding(
    ...
    Args:
        ...
        N_layers: Sequence of circuit depths considered in the scan.
    ...
    for s_val, pts in sorted(groups_j.items()):
        ...
        plt.plot(
            xs, ys, "-o", linewidth=2, markersize=5, label=rf"${sec_label} = {s_val}$"
        )
...
class CostFunctionContext(TypedDict):
    ...
    observables: Optional[Pauli]


class CostFunctionBuilder(Protocol):
    def __call__(
        self, context: CostFunctionContext, *args: Any, **kwargs: Any
    ) -> Callable[[list[float]], float]: ...


class VarianceNormalisation(Protocol):
    def __call__(
        self,
        n_qubits: int,
        depth: int,
...
def barren_plateaus_analysis(
    ...
    generate_params: Callable[[int, int], list[float]],
    generate_circuits: Callable[[int, int, str], QuantumCircuit],
    sampling: SamplingConfig | None = None,
    execution: ExecutionConfig | None = None,
    variance_normalization: VarianceNormalisation | None = None,
    **cost_kwargs: Any,
    ...
    Args:
        ...
        cost_function_builder: Function used to build the cost function for each
            experiment setting.
        generate_params: Function used to generate random circuit parameters.
        generate_circuits: Function used to generate circuits for the selected
            ansatz and settings.
        sampling: Sampling configuration. If None, default sampling settings are
            used.
        execution: Execution configuration. If None, default execution settings
            are used.
        variance_normalization: Optional function used to normalize the
            estimated variance before storing or plotting it.
        **cost_kwargs: Additional keyword arguments passed to the cost function
            builder.
    ...
    if experiment.analysis_type == "layerwise_qubits":
        ...
        if experiment.observables_list is not None and len(experiment.N_qubits) != len(
    ...
    if experiment.analysis_type == "layerwise_qubits_padding":
        ...
        if experiment.padding_types is None and experiment.initial_Pauli_string is None:
            pass

        elif (
            experiment.padding_types is None or experiment.initial_Pauli_string is None
        ):
            raise ValueError(
                "analysis_type='layerwise_qubits_padding' requires "
                "padding_types and initial_Pauli_string to either "
    ...
    if experiment.analysis_type == "joint_scaling_padding":
        ...
        if experiment.padding_types is None and experiment.initial_Pauli_string is None:
            ...
            raise ValueError(
                "analysis_type='joint_scaling_padding' requires "
                "padding_types and initial_Pauli_string to either "
                "both be None or both be defined."
            )
        ...
        for lay in experiment.N_layers:
            ...
            for nq, Pauli_string in iterable:
                ...
                point = run_single_point(
                    nq=nq,
                    lay=lay,
                    obs_list_builder=obs_builder_1,
                )
